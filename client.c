...
int main(int argc, char *argv[]) {
    if ((argc > 4) || (argc < 3)) {
        fprintf(stderr,
         "Uso: ./client <host> <puerto> [<archivo de entrada>]\n");
        return 1;
    }
 	client_t client;
    bool success_init = true;
    if (argc == 4) success_init &= 
            client_init(&client, argv[1], argv[2], argv[3]);
    else 
        success_init &= client_init(&client, argv[1], argv[2], NULL);
...
void client_run(client_t *self) {
    ...
    while (true) {
        ...
        char response[4] = {0};
        ssize_t rcvd_bytes =
             protocol_client_receive(self->protocol, response, 3);
        if (rcvd_bytes <= 0) {
